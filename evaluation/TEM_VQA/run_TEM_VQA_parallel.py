"""
Run TEM-VQA inference in parallel across multiple GPUs.

Each GPU processes a separate chunk of the input CSV.
Results are merged into a single output file upon completion.

Usage:
    python run_TEM_VQA_parallel.py \
        --inference_script inference_TEM_VQA_llava.py \
...
parser.add_argument('--inference_script', type=str, required=True,
                    help='Inference script to run (e.g. inference_TEM_VQA_llava.py)')
parser.add_argument('--input_csv',  type=str, required=True,
                    help='Path to input CSV (TEM_VQA.csv or TEM_MCQ.csv)')
...
model_tag   = script_name.replace('inference_TEM_VQA_', '')
...
for i, (start, end) in enumerate(ranges):
    chunk_output = os.path.join(
        args.output_dir,
        f"TEM_VQA_{model_tag}_chunk{i}_{start}_{end}.csv"
    )
...
if not files:
    ...
    final_output = os.path.join(args.output_dir, f"TEM_VQA_{model_tag}_outputs.csv")
